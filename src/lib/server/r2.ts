...
export async function deleteR2ObjectsWithPrefix(prefix: string) {
  ...
  do {
    const listResponse = await sendR2Command<{
      IsTruncated?: boolean;
      ...
      `delete-prefix:list:${normalizedPrefix}`
    );
    ...
    for (let index = 0; index < keys.length; index += 1000) {
      ...
      await sendR2Command(
        ...
        `delete-prefix:objects:${normalizedPrefix}`
      );
